...
public sealed class GravarPessoaUseCaseTest : BaseTest
{
    [Fact]
    public async Task Execute_SeExecutadoComSucesso_EntaoRetornaResponse()
    {
        ...
        var useCase = new GravarPessoaUseCase(mapper, repository);

        //Act
        var response = await useCase.Execute(new GravarPessoaRequest(expected));
